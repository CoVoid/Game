var display = (function() {
    ...
    function init() {
        if (!gl) return alert('ERR: WebGL not found!');
    ...
    function offset(x, y) {
        targetOffset[0] = x * 0.2;
        targetOffset[1] = y * 0.2;
    }

    function moveOffset() {
        let dx = targetOffset[0] - currentOffset[0];
        let dy = targetOffset[1] - currentOffset[1];
        currentOffset[0] = currentOffset[0] + 0.2 * dx;
        currentOffset[1] = currentOffset[1] + 0.2 * dy;
    }
    ...
    function render(time) {
        ...
        if (world) { // TODO this line might not be doing anything
            ...
            moveOffset();
            mat4.translate(vTransform, vTransform, currentOffset);
            mat4.invert(vTransform, vTransform);
